...
// Middleware para páginas renderizadas (não API)
exports.authPage = async (req, res, next) => {
  try {
    ...
    req.user = user;
    // Adicionar user ao res.locals para acesso nas views
    res.locals.user = {
      id: user._id,
      name: user.name,
      role: user.role
    ...
    console.error('Erro no middleware de autenticação de página:', err.message);
    res.clearCookie('token');
